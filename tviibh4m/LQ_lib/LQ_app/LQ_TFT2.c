...
#define USE_HARD_SPI 1  //使用硬件SPI则将其置1

unsigned char TFT2mode=1;

//龙邱TFT2 SPI口模块管脚顺序
#define TFT2SPI_CS    P07_3     // CS管脚 默认拉低，可以不用
#define TFT2SPI_SCK   P07_2     // SPI SCK管脚
#define TFT2SPI_SDI   P07_1     // SPI MOSI管脚
#define TFT2SPI_DC    P07_4     // D/C管脚
#define TFT2SPI_RST   P07_5     // RESET管脚
...
{
  Cy_SysTick_DelayInUs(ms*1000);
}

#define TFT2SPI_CS_H    LQ_Pin_Write(TFT2SPI_CS, 1);   /*!< CS管脚 */
#define TFT2SPI_SCK_H   LQ_Pin_Write(TFT2SPI_SCK, 1);  /*!< SCL管脚 */
#define TFT2SPI_SDI_H   LQ_Pin_Write(TFT2SPI_SDI, 1);  /*!< SDI管脚 */
#define TFT2SPI_DC_H    LQ_Pin_Write(TFT2SPI_DC, 1);   /*!< DC管脚 */
#define TFT2SPI_RST_H   LQ_Pin_Write(TFT2SPI_RST, 1);  /*!< RST管脚 */
                        
#define TFT2SPI_CS_L    LQ_Pin_Write(TFT2SPI_CS, 0);  /*!< CS管脚 */
#define TFT2SPI_SCK_L   LQ_Pin_Write(TFT2SPI_SCK, 0); /*!< SCL管脚 */
#define TFT2SPI_SDI_L   LQ_Pin_Write(TFT2SPI_SDI, 0); /*!< SDI管脚 */
#define TFT2SPI_DC_L    LQ_Pin_Write(TFT2SPI_DC, 0);  /*!< DC管脚 */
#define TFT2SPI_RST_L   LQ_Pin_Write(TFT2SPI_RST, 0); /*!< RST管脚 */
/*LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
@函数名称：write_command
@功能说明：往指定寄存器中写数据
...
void write_command(unsigned char LCD_Reg,uint16_t LCD_RegValue)
{
  ...
  TFT2SPI_Write_Word(LCD_RegValue);
...
@函数名称：TFT2SPI_Init
@功能说明：TFT2初始化
@参数说明：@param type ： 0:竖屏，引脚在屏幕的上方  1：横屏，引脚在屏幕左侧 2：竖屏，引脚在屏幕的下方 3：横屏，引脚在屏幕的右侧
@函数返回：无
@修改时间：2022/03/15
...
void TFT2SPI_Init(unsigned char type)
{
  ...
  LQ_GPIO_Init_Config(TFT2SPI_DC,  GPIO_OUT_STRONG);
  LQ_GPIO_Init_Config(TFT2SPI_RST, GPIO_OUT_STRONG);

#else
  //-----端口初始化----//
  LQ_GPIO_Init_Config(TFT2SPI_CS,  GPIO_OUT_STRONG);
  LQ_GPIO_Init_Config(TFT2SPI_DC,  GPIO_OUT_STRONG);
  LQ_GPIO_Init_Config(TFT2SPI_RST, GPIO_OUT_STRONG);
  LQ_GPIO_Init_Config(TFT2SPI_SCK, GPIO_OUT_STRONG);
  LQ_GPIO_Init_Config(TFT2SPI_SDI, GPIO_OUT_STRONG);
#endif
...
void TFT2SPI_P16x16Str(unsigned char x, unsigned char y, unsigned char *s_dat,unsigned short word_color,unsigned short back_color)
{
  ...
  while(s_dat[ii] != '\0')
  {
    ...
    {
      if(hanzi2_Idx[wm] == (uint8_t)s_dat[ii])
      {
...
{
  unsigned int i;
  uint16_t X_Start=xs, Y_Start=ys, Wide=w, High=h;
  TFT2SPI_Set_Pos(X_Start,Y_Start,X_Start+Wide-1,Y_Start+High);
  for(i=0;i<w*h;i++)
  {
