...
//龙邱OLED 模块管脚顺序
#define OLED_CK      P07_2     //OLED CK管脚
#define OLED_DI	     P07_1     //OLED DI管脚
#define OLED_RST     P07_5     //OLED RST管脚
#define OLED_DC	     P07_7     //OLED DC管脚
//#define OLED_CS	      P07_5     //OLED CS管脚 默认拉低，可以不用

#define OLED_DC_H  	LQ_Pin_Write(OLED_DC,GPIO_PIN_SET);
#define OLED_DC_L  	LQ_Pin_Write(OLED_DC,GPIO_PIN_RESET);

#define OLED_RST_H 	LQ_Pin_Write(OLED_RST,GPIO_PIN_SET);
#define OLED_RST_L 	LQ_Pin_Write(OLED_RST,GPIO_PIN_RESET);

#define OLED_DI_H  	LQ_Pin_Write(OLED_DI,GPIO_PIN_SET);
#define OLED_DI_L  	LQ_Pin_Write(OLED_DI,GPIO_PIN_RESET);

#define OLED_CK_H  	LQ_Pin_Write(OLED_CK,GPIO_PIN_SET);
#define OLED_CK_L  	LQ_Pin_Write(OLED_CK,GPIO_PIN_RESET);
...
void OLED_Init(void)
{ 
  LQ_GPIO_Init_Config(OLED_CK, GPIO_OUT_PP_UP);
  LQ_GPIO_Init_Config(OLED_DI, GPIO_OUT_PP_UP);
  LQ_GPIO_Init_Config(OLED_RST,GPIO_OUT_PP_UP);
  LQ_GPIO_Init_Config(OLED_DC, GPIO_OUT_PP_UP);
//  LQ_GPIO_Init_Config(OLED_CS,   GPIO_OUT_PP_DOWN);
...
因此如果画点 OLED_PutPixel(0,0); OLED_PutPixel(0,1); 只会显示点(0, 1) (0,0)则在画点(0,1)时清除掉
*************************************************************************/
...
*  修改时间：2023年11月23日
*  备    注：OLED是按字节显示的 一个字节8位，代表8个纵向像素点，
OLED_ClrDot(0, 0); //清除(0,0)-(0,7)8个点 左上角为0,0  右下角 127,63
*************************************************************************/
...
*  函数名称：OLED_Rectangle
*  功能说明：画矩形
*  参数说明：x1  : 0-127   左上角坐标（x1,y1）
             y1  : 0-63 
             x2  : 0-127   右下角坐标（x2，y2） 
             y2  : 0-63 
             gif : 是否开启动画效果 
*  函数返回：无
*  修改时间：2023年11月23日
...
{      
  int num=0;
  LQ_UART_Init_Config(UART7_RX_P02_0, UART7_TX_P02_1, 115200ul);       //初始化串口7
  GPIO_LED_Init();							//初始化LED	
  OLED_Init();                          //OLED初始化
  LQ_Printf("OLED Init is ok\n");
