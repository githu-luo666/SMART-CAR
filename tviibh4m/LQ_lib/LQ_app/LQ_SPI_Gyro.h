...
//*!**************************************
// MPU6500 的寄存器
#define ICM_SELF_TESTX_REG 0X0D  // 寄存器 X
#define ICM_SELF_TESTY_REG 0X0E  // 寄存器 Y
#define ICM_SELF_TESTZ_REG 0X0F  // 寄存器 Z
#define ICM_SELF_TESTA_REG 0X10  // 寄存器 A
#define ICM_SAMPLE_RATE_REG 0X19 // 采样频率分频器
#define ICM_CFG_REG 0X1A         // 配置寄存器
#define ICM_GYRO_CFG_REG 0X1B    // 陀螺仪配置寄存器
#define ICM_ACCEL_CFG_REG 0X1C   // 加速度计寄存器
#define ICM_MOTION_DET_REG 0X1F  // 运动检测寄存器
#define ICM_FIFO_EN_REG 0X23     // FIFO 功能寄存器

#define ICM_I2CMST_STA_REG 0X36 // IIC 主机状态寄存器
#define ICM_INTBP_CFG_REG 0X37  // 中断/引脚配置寄存器
#define ICM_INT_EN_REG 0X38     // 中断功能寄存器
#define ICM_INT_STA_REG 0X3A    // 中断状态寄存器

#define ICM_ACCEL_XOUTH_REG 0X3B // 加速度值, X 高 8 位寄存器
#define ICM_ACCEL_XOUTL_REG 0X3C // 加速度值, X 低 8 位寄存器
#define ICM_ACCEL_YOUTH_REG 0X3D // 加速度值, Y 高 8 位寄存器
#define ICM_ACCEL_YOUTL_REG 0X3E // 加速度值, Y 低 8 位寄存器
#define ICM_ACCEL_ZOUTH_REG 0X3F // 加速度值, Z 高 8 位寄存器
#define ICM_ACCEL_ZOUTL_REG 0X40 // 加速度值, Z 低 8 位寄存器


#define ICM_ACCEL_DATA_X1     0X1F    // 加速度值, X 高 8 位寄存器  ICM42605
#define ICM_ACCEL_DATA_X0     0X20    // 加速度值, X 低 8 位寄存器
#define ICM_ACCEL_DATA_Y1     0X21    // 加速度值, Y 高 8 位寄存器
#define ICM_ACCEL_DATA_Y0     0X22    // 加速度值, Y 低 8 位寄存器
#define ICM_ACCEL_DATA_Z1     0X23    // 加速度值, Z 高 8 位寄存器
#define ICM_ACCEL_DATA_Z0     0X24    // 加速度值, Z 低 8 位寄存器

#define ICM_TEMP_OUTH_REG 0X41 // 温度值高 8 位寄存器
#define ICM_TEMP_OUTL_REG 0X42 // 温度值低 8 位寄存器

#define ICM_GYRO_XOUTH_REG 0X43 // 陀螺仪值, X 高 8 位寄存器
#define ICM_GYRO_XOUTL_REG 0X44 // 陀螺仪值, X 低 8 位寄存器
#define ICM_GYRO_YOUTH_REG 0X45 // 陀螺仪值, Y 高 8 位寄存器
#define ICM_GYRO_YOUTL_REG 0X46 // 陀螺仪值, Y 低 8 位寄存器
#define ICM_GYRO_ZOUTH_REG 0X47 // 陀螺仪值, Z 高 8 位寄存器
#define ICM_GYRO_ZOUTL_REG 0X48 // 陀螺仪值, Z 低 8 位寄存器

#define ICM_I2CSLV0_DO_REG 0X63 // IIC 从机 0 数据寄存器
#define ICM_I2CSLV1_DO_REG 0X64 // IIC 从机 1 数据寄存器
#define ICM_I2CSLV2_DO_REG 0X65 // IIC 从机 2 数据寄存器
#define ICM_I2CSLV3_DO_REG 0X66 // IIC 从机 3 数据寄存器

#define ICM_I2CMST_DELAY_REG 0X67 // IIC 主机延迟管理寄存器
#define ICM_SIGPATH_RST_REG 0X68  // 信号路径复位寄存器
#define ICM_MDETECT_CTRL_REG 0X69 // 运动检测控制器
#define ICM_USER_CTRL_REG 0X6A    // 用户控制寄存器
#define ICM_PWR_MGMT1_REG 0X6B    // 电源管理寄存器 1
#define ICM_PWR_MGMT2_REG 0X6C    // 电源管理寄存器 2
#define ICM_FIFO_CNTH_REG 0X72    // FIFO 计数寄存器高 8 位
#define ICM_FIFO_CNTL_REG 0X73    // FIFO 计数寄存器低 8 位
#define ICM_FIFO_RW_REG 0X74      // FIFO 读写寄存器
#define WHO_AM_I 0X75             // 设备 ID 寄存器

// LSM6D 陀螺仪寄存器定义
#define DRV_ID_LSM6D    0x6B    // 设备 ID
#define WHO_AM_LSM6D    0x0F    // 设备 ID 寄存器
...
#define LSM6DSR_CTRL1_XL 0X10       // 加速度配置寄存器 1 (r/w) bit1:0: 第一级数据输出选择。1: LPF2 第二级输出选择 bit[2:3]: 加速度量程选择，默认为 00: ±2g 01: ±16g 10: ±4g 11: ±8g
...
/* @brief    测试 20602 SPI 通信方式*/
void Test_SPI_Gyro(void);
/*@brief    初始化 ICM20602 或 ICM20602*/
